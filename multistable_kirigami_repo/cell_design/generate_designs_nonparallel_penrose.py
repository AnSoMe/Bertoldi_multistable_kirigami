...
d1_reg = 3. #match length for general struts between cells
d1_alt = 3. #match length for special struts between cells
...
tiledict =  generate_tiledict_penrose(Penrose_length)
...
for ki, k in enumerate(tiledict.keys()):
    ...
    newargs = [a, b, gamma, s, d1, d2, h, vocal, b2, gamma2, s2, Dweight, Aweight, Bweight, Cweight, True]
    finalcost, d2_opt = cost_function_fabrication_pairwise_v5(res.x, newargs)
    newargs = [a, b, gamma, s, d1, d2, h, vocal, b2, gamma2, s2, Dweight, Aweight, Bweight, Cweight, False]
    finalcost = cost_function_fabrication_pairwise_v5(res.x, newargs)
    d2_opt = d2
...
ax.set_ylabel(r'mm')
ax.axis('equal')
drawtiledict = tesselate_Penrose_cells(newtiledict, tiledict)
