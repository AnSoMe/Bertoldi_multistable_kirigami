...
# ktauprefactors = [1, 2, 5, 20]
# kthetaprefactors = [0.1,  0.2, 0.5,  1]
...
for ki, ktauprefactor in enumerate(ktauprefactors):
    for khi, kthetaprefactor in enumerate(kthetaprefactors):
        print(ki, khi)
        figname = r'./energy_sweep_ktautest_ktautest_{:.2f}_kthetatest_{:.2f}_test16'.format(ktauprefactor, kthetaprefactor)
        
        length_b = 24 #mm
        

        # kthetaprefactors=0.1
        ...
        hao = overview_axs[ki][khi].scatter(gamma_RU_values, np.array(lengths_RU_b), c=np.array(py_values), 
                                            cmap='YlGn',vmin=-0.1, vmax=13, alpha=0.3, zorder=-1000, lw=0)
        #alternatively: make contour plot 
        ...
        hao = overview_axs[ki][khi].contour(xi, yi, zi_interp, 
        ...
        ax3.scatter(gamma_RU_values, np.array(lengths_RU_b), np.array(py_values), c=np.array(py_values), 
                                            cmap='YlGn',vmin=-0.1, vmax=13, alpha=0.2, zorder=-1000)
        ax3.contour(xi, yi, zi_interp,
                    cmap='YlGn',vmin=-0.1, vmax=13, levels=20)      
        ...
        overview_axs[ki][khi].set_xlim(0, np.pi/2.)
        overview_axs[ki][khi].set_xticks([0,0.5,1,1.5])
